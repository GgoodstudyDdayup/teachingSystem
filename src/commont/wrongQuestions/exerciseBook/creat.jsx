...
import { create_question } from '../../../axios/http'
...
export default class EditorDemo extends React.Component {
    ...
    switchState = (value, data) => {
        ...
        if (this.props.editorDParams.template_id === '') {
            ...
            switch (value) {
                ...
                case '3':
                    const list = data.ques_answer.split('')
                    const ques_options = this.props.editorDParams.ques_options.match(/<div(([\s\S])*?)<\/div>/g)
    ...
    tijiaoshiti = () => {
        const params = { ...this.state.params }
        console.log(params)
        if(params.ques_answer ===''||params.ques_content ===''||params.ques_analysis ===''){
            message.warning('请填写信息')
            return
        }
        const that = this
        if (this.props.btnChange) {
            confirm({
                title: '是否要提交试题？',
                ...
                <div >
                    <div className="m-flex m-bottom" style={{ alignItems: 'center' }}>
                        ...
                        <div className="m-left">
                            {this.props.editorDParams.template_id === '' ?
                                ...
                                <Radio.Group onChange={this.onchangetemplate} value={this.props.editorDParams.template_id} disabled={true}>
                                    <Radio value='1'>填空题模板</Radio>
                                    <Radio value='2'>解答题模板</Radio>
                                    <Radio value='3'>选择题模板</Radio>
                                    <Radio value='4'>判断题模板</Radio>
                    ...
                    {this.props.editorDParams.template_id === '' ? this.switchState(this.state.params.template_id, this.state.params) : this.switchState(this.props.editorDParams.template_id, this.props.editorDParams)}
                </div>
